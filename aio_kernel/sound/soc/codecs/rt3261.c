...
#define RT3261_DP703_VOL_CONTROL 1
#define RT3261_SPK_VOL_0DB  8
#define RT3261_SPK_VOL_MAX  0x27
...
static int hdmi_in_debug = 1;
static int rt3261_debug_rw = 0;
static struct rt3261_init_reg init_list[] = {
#if defined (CONFIG_MACH_RK3188_AIO) 
	{RT3261_GEN_CTRL1	, 0x0f01},//fa[12:13] = 0'b; open dmic path
#else
...
#if defined (CONFIG_MACH_RK3188_AIO) 
    {RT3261_MONO_ADC_MIXER, 0x5050}, //DMIC L R 
    {RT3261_DMIC, 0xBD00},           //enable DMIC CLK output
    {RT3261_GPIO_CTRL1, 0x4400},     //select DMIC CLK 
    {RT3261_DSP_PATH2	, 0x0c00},   
#else
...
extern int scaler_get_hdmi_in_status(int scaler_id);
		...
		unsigned int reg)
{
    ...
    if ((1 == scaler_get_hdmi_in_status(1)) && (0 == rt3261_debug_rw))
    {
        if (1 == hdmi_in_debug)
        {
            printk("%s:r reg[0x%2x]\n", __func__, reg);  
        }
		...
		unsigned int value)
{
    ...
    if ((1 == scaler_get_hdmi_in_status(1)) && (0 == rt3261_debug_rw))
    {
        if (1 == hdmi_in_debug)
        {
            printk("%s:w reg[0x%2x] = 0x%4x{HDMI IN}\n", __func__, reg, value);  
        }
...
//bard 8-26 e
		...
		//rt3261_index_update_bits(codec,
		//	RT3261_CLSD_INT_REG1, 0xf000, 0xf000);      //add by hulin
	...
	case SND_SOC_DAPM_PRE_PMD:
		snd_soc_update_bits(codec, RT3261_SPK_VOL,
			...
			RT3261_L_MUTE | RT3261_R_MUTE);
		//rt3261_index_update_bits(codec,
		//	RT3261_CLSD_INT_REG1, 0xf000, 0x0000);    //add by hulin
		snd_soc_update_bits(codec, RT3261_PWR_DIG1,
			RT3261_PWR_CLS_D, 0);
		break;
...
static int rt3261_print_all_reg(void)
{
    struct snd_soc_codec *codec = rt3261_codec;
    int value = 0;
    int i = 0;
    ...
    for (i = 0; i < 0x100; i++)
    {
        ...
        value = snd_soc_read(rt3261_codec, i);
...
//#define AUDIO_DENOISING_TEST 1
#ifdef AUDIO_DENOISING_TEST

struct workqueue_struct *denoising_workqueue;
struct delayed_work denoising_delayed_work;

static void denoising_work(struct work_struct *work)
{
...
#if 0
    /* ÉèÖÃI2CÍ¨µÀµ½RK3188£¬ ÅäÖÃCODEC³öÉùÒô*/    
    gpio_request(RK30_PIN0_PA3, "i2c_switch1");
	gpio_direction_output(RK30_PIN0_PA3, GPIO_HIGH);

    rt3261_print_all_reg();
...
static int rt3261_probe(struct snd_soc_codec *codec)
{
    ...
    denoising_workqueue = create_singlethread_workqueue("denoising_workqueue");
    INIT_DELAYED_WORK(&denoising_delayed_work, denoising_work);
...
static int rt3261_get_freq(int hdmi_freq)
{
    int freq = 11289600;
    
    switch (hdmi_freq)
    {
        case DP703_E_HDMI_FREQ_32000:
            printk("hdmi freq:32KHZ\n");
            freq = 8192000;
            break;
        case DP703_E_HDMI_FREQ_44100:
            printk("hdmi freq:44.1KHZ\n");
            freq = 11289600;
            break;
        case DP703_E_HDMI_FREQ_48000:
            printk("hdmi freq:48KHZ\n");
            freq = 12288000;
            break;
        case DP703_E_HDMI_FREQ_88200:
            printk("hdmi freq:88.2KHZ\n");
            freq = 11289600 * 2;
            break;
        case DP703_E_HDMI_FREQ_96000:
            printk("hdmi freq:96KHZ\n");
            freq = 12288000 * 2;
            break;
        case DP703_E_HDMI_FREQ_176400:
            printk("hdmi freq:176.4KHZ\n");
            freq = 11289600 * 4;
            break;
        case DP703_E_HDMI_FREQ_192000:
            printk("hdmi freq:192KHZ\n");
            freq = 12288000 * 4;
            break;
        default:
            freq = -1;
...
int rt3261_channel_switch(int channel, int hdmi_freq)
{
    struct snd_soc_codec *codec = NULL;
    struct rt3261_pll_code pll_code;
    unsigned int freq_in = 11289600;
    unsigned int freq_out = 0; /*12288000 * 2;freq_in * 2;*/
    static unsigned int s_dp703_vol_value = RT3261_SPK_VOL_0DB;
    static unsigned int s_dp703_mute_status = 0;
    static unsigned int s_codec_pll_ctl1 = 0;
    static unsigned int s_codec_pll_ctl2 = 0;
    unsigned int value = 0;
    int ret = 0;
    int i = 0;

    if ((NULL == rt3261_codec) || (channel > RT3261_DP703_TO_CODEC))
    {
        printk("params error! channel = %d\n", channel);
        return -1;
    }

    codec = rt3261_codec;
    if (RT3261_RK3188_TO_CODEC == channel)
    {
#ifdef RT3261_DP703_VOL_CONTROL
        /* save dp703 mute status */
        value = snd_soc_read(codec, RT3261_SPK_VOL);
        s_dp703_mute_status = (value & RT3261_L_MUTE) >> RT3261_L_MUTE_SFT;
        printk("read s_dp703_mute_status = %d\n", s_dp703_mute_status);
#endif

        snd_soc_update_bits(codec, RT3261_SPK_VOL,
            RT3261_L_MUTE | RT3261_R_MUTE,
            RT3261_L_MUTE | RT3261_R_MUTE); /* mute spk */

        /* recovery codec pll setup */
        snd_soc_write(codec, RT3261_PLL_CTRL1, s_codec_pll_ctl1);
        snd_soc_write(codec, RT3261_PLL_CTRL2, s_codec_pll_ctl2);
        
#ifdef RT3261_DP703_VOL_CONTROL
        /* save dp703 volume status */
        value = snd_soc_read(codec, RT3261_SPK_VOL);
        s_dp703_vol_value = value & RT3261_R_VOL_MASK;

        printk("read s_dp703_vol_value = %d\n", s_dp703_vol_value);
        if ((s_dp703_vol_value < RT3261_SPK_VOL_0DB)
            || (s_dp703_vol_value > RT3261_SPK_VOL_MAX))
        {
            printk("3188 dp703_vol_value = 0x%X error!\n", s_dp703_vol_value);
        }
        else
        {
            for (i = 0; i < s_dp703_vol_value - RT3261_SPK_VOL_0DB; i++)
            {
                snd_soc_update_bits(codec, RT3261_SPK_VOL,
                    RT3261_L_VOL_MASK | RT3261_R_VOL_MASK , 
                    (s_dp703_vol_value - 1 - i) << RT3261_L_VOL_SFT 
                    | (s_dp703_vol_value - 1 - i) << RT3261_R_VOL_SFT);
            }
        }
#endif        
        /* switch codec internal path */
        snd_soc_update_bits(codec, RT3261_MONO_DAC_MIXER,
            RT3261_M_DAC_L1_MONO_L | RT3261_M_DAC_L2_MONO_L
            | RT3261_M_DAC_R1_MONO_R | RT3261_M_DAC_R2_MONO_R,
            RT3261_M_DAC_L1_MONO_L | RT3261_M_DAC_R1_MONO_R); /*1: mute */

        snd_soc_dapm_sync(&codec->dapm);
        printk("i2s path switch to rk3188\n");
    }
    else
    {
        snd_soc_update_bits(codec, RT3261_SPK_VOL,
        ...
        /* Insert the HDMI in the system suspend , resume codec */
        rt3261_set_bias_level(codec, SND_SOC_BIAS_STANDBY);
        
#ifdef RT3261_DP703_VOL_CONTROL
        /* recover dp703 volume status */
        printk("s_dp703_vol_value = 0x%x\n", s_dp703_vol_value);
        if ((s_dp703_vol_value < RT3261_SPK_VOL_0DB) 
            || (s_dp703_vol_value > RT3261_SPK_VOL_MAX))
        {
            printk("dp703_vol_value = 0x%X error!\n", s_dp703_vol_value);
        }
        else
        {
            for (i = 0; i < s_dp703_vol_value - RT3261_SPK_VOL_0DB; i++)
            {
                ...
                snd_soc_update_bits(codec, RT3261_SPK_VOL,
                    ...
                    (RT3261_SPK_VOL_0DB + i + 1) << RT3261_L_VOL_SFT 
                    | (RT3261_SPK_VOL_0DB + i + 1) << RT3261_R_VOL_SFT);
            }
        ...
        snd_soc_update_bits(codec, RT3261_MONO_DAC_MIXER,
            RT3261_M_DAC_L1_MONO_L | RT3261_M_DAC_L2_MONO_L
            | RT3261_M_DAC_R1_MONO_R | RT3261_M_DAC_R2_MONO_R,
            RT3261_M_DAC_L2_MONO_L | RT3261_M_DAC_R2_MONO_R);

        /* power on path power */
        snd_soc_update_bits(codec, RT3261_PWR_DIG1, 
            RT3261_PWR_I2S1 | RT3261_PWR_DAC_L1 | RT3261_PWR_DAC_R1 
            | RT3261_PWR_DAC_L2 | RT3261_PWR_DAC_R2 | RT3261_PWR_CLS_D,
            RT3261_PWR_I2S1 | RT3261_PWR_DAC_L1 | RT3261_PWR_DAC_R1 
            | RT3261_PWR_DAC_L2 | RT3261_PWR_DAC_R2 | RT3261_PWR_CLS_D); /* 61: 98c1 */

        snd_soc_update_bits(codec, RT3261_PWR_ANLG2, 
            RT3261_PWR_MB1 | RT3261_PWR_MB2 | RT3261_PWR_PLL,
            RT3261_PWR_MB1 | RT3261_PWR_MB2 | RT3261_PWR_PLL);     /* 64: 0x0e00 */  

        snd_soc_update_bits(codec, RT3261_PWR_MIXER, 
            RT3261_PWR_SM_L | RT3261_PWR_SM_R,
            RT3261_PWR_SM_L | RT3261_PWR_SM_R);   /* 65: 0x3000*/

        snd_soc_update_bits(codec, RT3261_PWR_MIXER,
            RT3261_PWR_SV_L | RT3261_PWR_SV_R,
            RT3261_PWR_SV_L | RT3261_PWR_SV_R);   /* 66: 0xc000 */       
        
        /* set codec clk */
        snd_soc_update_bits(codec, RT3261_GLB_CLK,
		RT3261_SCLK_SRC_MASK, RT3261_SCLK_SRC_PLL1); 

        ret = rt3261_get_freq(hdmi_freq);
        if (ret < 0)
        {
            printk("%s: line=%d rt3261_get_freq fail!\n", __func__, __LINE__);
            return -1;
        }
        ...
        s_codec_pll_ctl1 = snd_soc_read(codec, RT3261_PLL_CTRL1);
        s_codec_pll_ctl2 = snd_soc_read(codec, RT3261_PLL_CTRL2);
        printk("pll_ctl1 = 0x%x, pll_ctl2 = 0x%x\n", s_codec_pll_ctl1, s_codec_pll_ctl2);
        ...
        printk("k = 0x%x, mbp = 0x%x, m = 0x%x, n = 0x%x\n",
            pll_code.k_code, pll_code.m_bp, pll_code.m_code, pll_code.n_code);

#ifdef AUDIO_DENOISING_TEST
        queue_delayed_work(denoising_workqueue, &denoising_delayed_work, msecs_to_jiffies(3000));
#endif

        snd_soc_dapm_sync(&codec->dapm);

#ifdef RT3261_DP703_VOL_CONTROL
        /* recover dp703 mute status */
        printk("s_dp703_mute_status = %d\n", s_dp703_mute_status);
        if (0 == s_dp703_mute_status)
        {
            snd_soc_update_bits(codec, RT3261_SPK_VOL,
                RT3261_L_MUTE | RT3261_R_MUTE,
                0);                                        /* unmute spk */
        ...
        printk("i2s path switch to dp703\n");
    }
    
    return 0;
}
EXPORT_SYMBOL_GPL(rt3261_channel_switch);
#endif
		...
		unsigned long len, void *data)
{
	...
	switch(cookie_pot[0])
	{
        ...
        case 'M':
			for(i = 0; i < 0x100; i++)
			{
				value = rt3261_read(rt3261_codec, i);
				printk("rt3261_read:0x%04x = 0x%04x\n",i,value);
			}
			break;
        case 'h':
            ...
            hdmi_in_debug = simple_strtol(start,NULL,16) ? 1 : 0;
            printk("hdmi_in_debug = %d\n", hdmi_in_debug);
            break;
        case 't':
            start = ++cookie_pot;
            while(' ' == *start)
            ...
            rt3261_debug_rw = simple_strtol(start,NULL,16) ? 1 : 0;
            printk("rt3261_debug_rw = %d\n", rt3261_debug_rw);
            break;
		case 'r':
		case 'R':
		...
		default:
            ...
            printk("-->'m&&M' display all reg value\n");
