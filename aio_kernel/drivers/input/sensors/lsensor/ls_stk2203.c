...
{	
	...
	result = sensor_write_reg(client, ALS_THDH1, 1);//it is important,if not then als can not trig intterupt
	if(result)
	{
		printk("%s:line=%d,error\n",__func__,__LINE__);
		return result;
...
struct sensor_operate light_stk2203_ops = {
	.name				= "ls_stk2203",
	.type				= SENSOR_TYPE_LIGHT,	//sensor type and it should be correct
	.id_i2c				= LIGHT_ID_STK2203,	//i2c id number
...
{
	return &light_stk2203_ops;
}


static int __init light_stk2203_init(void)
{
...
static void __exit light_stk2203_exit(void)
{
	struct sensor_operate *ops = light_get_ops();
	int type = ops->type;
...
module_init(light_stk2203_init);
module_exit(light_stk2203_exit);
